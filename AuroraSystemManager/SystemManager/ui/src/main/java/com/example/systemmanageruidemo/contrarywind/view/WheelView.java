package com.example.systemmanageruidemo.contrarywind.view;
...
import com.example.systemmanageruidemo.contrarywind.interfaces.IPickerViewData;
import com.example.systemmanageruidemo.contrarywind.listener.LoopViewGestureListener;
import com.example.systemmanageruidemo.contrarywind.listener.OnItemSelectedListener;
import com.example.systemmanageruidemo.contrarywind.timer.InertiaTimerTask;
import com.example.systemmanageruidemo.contrarywind.timer.MessageHandler;
import com.example.systemmanageruidemo.contrarywind.timer.SmoothScrollTimerTask;
...
public class WheelView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        while (counter < itemsVisible) {
            ...
            // 弧度转换成角度(把半圆以Y轴为轴心向右转maxAngle度，使其处于第一象限及第四象限
            // angle [-maxAngle°,maxAngle°]
            float angle = (float) (maxAngle - (radian / Math.PI) * maxAngle*2);//item第一项,从maxAngle度开始，逐渐递减到 -maxAngle度

            // 计算取值可能有细微偏差，保证负maxAngle°到maxAngle°以外的不绘制
            if (angle > maxAngle || angle < -maxAngle) {
                ...
                float offsetCoefficient = (float) Math.pow(Math.abs(angle) / maxAngle, 2.2);
    ...
    private void setOutPaintStyle(float offsetCoefficient, float angle) {
        ...
        // 控制透明度
        int alpha = isAlphaGradient ? (int) ((maxAngle - Math.abs(angle)) / maxAngle * 255) : 255;
