...
public class SystemCheck {
     ...
     * 可直接清理的垃圾，跳转到垃圾清理
     */
    public static void startRubbishCleanForResult(BaseActivity context, int requestCode) {
        Intent intent = new Intent();
        try {
     ...
     * 跳转到流量管理界面，设置流量套餐
     */
    public static void startSetTrafficSet(BaseActivity context, int requestCode){
        SIMInfoWrapper simInfo = SIMInfoWrapper.getDefault(context);
        int defaultDataSubId = simInfo.getSimIndex_CurrentNetworkActivated();
        if (defaultDataSubId == -1){
            defaultDataSubId = simInfo.getInsertedSimInfo().get(0).mSlot;
     ...
     */
    public static void startInternetControlActivity(BaseActivity context){
     ...
     **/
    public static  void uninstallNotFrequentlyUsedApps(BaseActivity context){
        Intent intent = new Intent();
