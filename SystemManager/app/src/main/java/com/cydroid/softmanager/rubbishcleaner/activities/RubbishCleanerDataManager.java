...
package com.cydroid.softmanager.rubbishcleaner.activities;
...
import com.cydroid.softmanager.rubbishcleaner.common.DeeplyCleanTypeConst;
import com.cydroid.softmanager.rubbishcleaner.common.MsgConst;
import com.cydroid.softmanager.rubbishcleaner.common.RubbishInfo;
import com.cydroid.softmanager.rubbishcleaner.interfaces.RubbishCleanerDelListener;
import com.cydroid.softmanager.rubbishcleaner.interfaces.RubbishCleanerScanListener;
import com.cydroid.softmanager.rubbishcleaner.interfaces.RubbishCleanerStrategy;
import com.cydroid.softmanager.rubbishcleaner.util.FileIconHelper;
import com.cydroid.softmanager.rubbishcleaner.util.FileNameUtils;
import com.cydroid.softmanager.rubbishcleaner.util.MimeTypeUtils;
import com.cydroid.softmanager.utils.ApkInfoHelper;
import com.cydroid.softmanager.utils.FileUtil;
...
public class RubbishCleanerDataManager {
    ...
    private final ArrayList<RubbishCleanerScanListener> mRubbishCleanerScanListeners = new ArrayList<RubbishCleanerScanListener>();
    private final ArrayList<RubbishCleanerDelListener> mRubbishCleanerDelListeners = new ArrayList<RubbishCleanerDelListener>();
    ...
    private final ArrayList<RubbishInfo> mCacheList = new ArrayList<RubbishInfo>();
    private final ArrayList<RubbishInfo> mAdList = new ArrayList<RubbishInfo>();
    private final ArrayList<RubbishInfo> mResidualList = new ArrayList<RubbishInfo>();
    private final ArrayList<RubbishInfo> mApkList = new ArrayList<RubbishInfo>();
    private final ArrayList<RubbishInfo> mBigfileList = new ArrayList<RubbishInfo>();
    private final ArrayList<ArrayList<RubbishInfo>> mGroupList = new ArrayList<ArrayList<RubbishInfo>>();

    private final ArrayList<GroupKeyInfo> mCacheKeyList = new ArrayList<GroupKeyInfo>();
    private final HashMap<String, ArrayList<RubbishInfo>> mCacheListGroupMap = new HashMap<String, ArrayList<RubbishInfo>>();

    private final ArrayList<GroupKeyInfo> mBigFileKeyList = new ArrayList<GroupKeyInfo>();
    private final HashMap<String, ArrayList<RubbishInfo>> mBigFileListGroupMap = new HashMap<String, ArrayList<RubbishInfo>>();
    ...
    public boolean isGroupOutOfBound(int group) {
        ...
        Log.d(TAG, "isGroupOutOfBound size " + size);
    ...
    private void startScanAd(final int token) {
        final IAdDirCallback.Stub adCallback = new IAdDirCallback.Stub() {
            ...
            public boolean onScanItem(String desc, int nProgressIndex) throws RemoteException {
                if (isServing()) {
                    sendMsg2Handler(MsgConst.SCAN_ITEM, -1, 0, desc);
                }
                return false;
            }
            ...
            public void onFindAdDir(String name, String dirPath) throws RemoteException {
                if (mAdList != null && isServing()) {
                    ...
                    mAdList.add(info);
                    sendMsg2Handler(MsgConst.FIND_ITEM, -1, token, Long.valueOf(info.size));
                }
            }
    ...
    private void callRubbishCleanerScanItem(Object obj) {
        for (RubbishCleanerScanListener listener : mRubbishCleanerScanListeners) {
            listener.onScanItem(obj);
        }
    }

    private void callRubbishCleanerScanFindItem(Object obj) {
        for (RubbishCleanerScanListener listener : mRubbishCleanerScanListeners) {
            listener.onFindItem(obj);
        }
    }
        ...
        public void dispatchMessage(Message msg) {
            switch (msg.what) {
                ...
                case MsgConst.SCAN_ITEM:
                    callRubbishCleanerScanItem(msg.obj);
                    break;
                case MsgConst.FIND_ITEM:
                    if (msg.arg2 == mToken) {
                        mTotalSize += longValue(msg.obj);
                        callRubbishCleanerScanFindItem(msg.obj);
                    }
                    break;
                case MsgConst.END:
                    if (msg.arg2 == mToken) {
